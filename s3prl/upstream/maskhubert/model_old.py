...
class CustomStudentModelConfig(FairseqDataclass):
    ...
    teacher_encoder_embed_dim: int = field(
        default=768, metadata={"help": "teacher encoder embedding dimension"}
    )
...
class CustomStudentModel(BaseFairseqModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.post_extract_proj = (
            nn.Linear(self.embed, cfg.teacher_encoder_embed_dim)
            if self.embed != cfg.teacher_encoder_embed_dim
            else None
        )
        self.post_mask_proj = (
            nn.Linear(cfg.teacher_encoder_embed_dim, cfg.encoder_embed_dim)
            if cfg.teacher_encoder_embed_dim != cfg.encoder_embed_dim
            else None
        )
        ...
        self.mask_emb = nn.Parameter(
            torch.FloatTensor(cfg.teacher_encoder_embed_dim).uniform_()
        )
    ...
    def forward(
        ...
        if self.post_mask_proj is not None:
            features = self.post_mask_proj(features)
